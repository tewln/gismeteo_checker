...
def save_weather_to_db(weather_data:dict):
    ...
    row_in_db = dict()
    row_in_db.update({date: dict()})
    for time_of_day, data in weather_data[date].items():
        row_in_db[date].update({time_of_day : data})
    
    connection = get_db_connection()
    cursor = connection.cursor()
    
    cursor.execute("""
                    INSERT INTO weather (date,  temperature_morning, temperature_evening, pressure_morning, pressure_evening, cloudiness_morning, cloudiness_evening, wind_speed_morning, wind_speed_evening, wind_direction_morning, wind_direction_evening)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    ON CONFLICT (date, time_of_day) DO NOTHING;
                   """, (date, row_in_db[date][0]["temperature"], row_in_db[date][1]["temperature"], row_in_db[date][0]["pressure"], row_in_db[date][1]["pressure"], row_in_db[date][0]["cloudiness"], row_in_db[date][1]["cloudiness"], row_in_db[date][0]["wind_speed"], row_in_db[date][1]["wind_speed"], row_in_db[date][0]["wind_direction"], row_in_db[date][1]["wind_direction"])
                   )
    connection.commit()
    cursor.close()
    connection.close()
...
def get_weather(user_date:str, period:int):
    ...
    for row in rows:
        answer.update({row[0] : {"День" : {"temperature" : row[1],
                                            "pressure" : row[3],
                                            "cloudiness" : row[5],
                                            "precipitation" : row[7],
                                            "wind_speed" : row[9],
                                            "wind_direction" : row[11]},
                                 "Вечер" : {"temperature" : row[2],
                                            "pressure" : row[4],
                                            "cloudiness" : row[6],
                                            "precipitation" : row[8],
                                            "wind_speed" : row[10],
...
def fetch_weather_data():
    ...
    for i in range(0,2):
        weather_per_day[current_date].update({i : {"temperature" : temperatures[i],
...
def main():
    while(True):
        ...
        print((next_wakeup - now).total_seconds())
